...
mu_MR_sumSE_DNN = dataDNN.mu_MR_sumSE_DNN;
...
for n = 1:NoOfSetups
    mu_MR_sumSE_DNN(:,:,n) = mu_MR_sumSE_DNN(:,:,n) .* sqrt(Pmax/(max(sum(mu_MR_sumSE_DNN(:,:,n).^2,1))));
end
...
for l = 1:cluster_size:L
    ...
    for c = 1:cluster_size
        store = reshape(mu_MR_sumSE_DNN(:,l+c-1,:),K,[]).';
        store(big_values,:) = [];
    ...
    save("MR_sumSE_CANN_"+l+".mat","parameters");
end
